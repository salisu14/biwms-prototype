...
class SalesOrder extends Model
{
    ...
    public function lines(): HasMany { return $this->hasMany(SalesOrderLine::class, 'sales_order_id')->orderBy('line_number'); }
    public function creator(): BelongsTo { return $this->belongsTo(User::class, 'created_by'); }
    public function salesperson(): BelongsTo { return $this->belongsTo(User::class, 'salesperson_id'); }
    public function warehouseWorker(): BelongsTo { return $this->belongsTo(User::class, 'assigned_warehouse_worker_id'); }
    public function approver(): BelongsTo { return $this->belongsTo(User::class, 'approved_by'); }
    public function canceller(): BelongsTo { return $this->belongsTo(User::class, 'cancelled_by'); }
    public function generalBusinessPostingGroup(): BelongsTo { return $this->belongsTo(GeneralBusinessPostingGroup::class); }
    public function customerPostingGroup(): BelongsTo { return $this->belongsTo(CustomerPostingGroup::class); }
    public function pricingGroup(): BelongsTo { return $this->belongsTo(PricingGroup::class); }
    public function warehouseShipments(): HasMany { return $this->hasMany(WarehouseShipment::class, 'source_document_id')->where('source_document', 'SALES_ORDER'); }
    public function postedInvoices(): HasMany { return $this->hasMany(PostedSalesInvoice::class, 'order_id'); }

    // ==================== SCOPES ====================
    public function scopeOpen($query) { return $query->whereNotIn('status', ['CLOSED', 'CANCELLED', 'FULLY_INVOICED']); }
    public function scopeReadyToShip($query) { return $query->whereIn('status', ['APPROVED', 'RELEASED', 'PARTIALLY_SHIPPED']); }
    public function scopeOverdue($query) { return $query->where('promised_delivery_date', '<', now())->whereNotIn('status', ['SHIPPED', 'INVOICED', 'CLOSED', 'CANCELLED']); }
    public function scopeForCustomer($query, int $customerId) { return $query->where('customer_id', $customerId); }
    public function scopeForSalesperson($query, int $salespersonId) { return $query->where('salesperson_id', $salespersonId); }
    ...
    public function getCanEditAttribute(): bool { return in_array($this->status, ['DRAFT', 'PENDING_APPROVAL']); }
    public function getCanApproveAttribute(): bool { return $this->status === SalesOrderStatus::PENDING_APPROVAL; }
    public function getCanReleaseAttribute(): bool { return $this->status === SalesOrderStatus::APPROVED; }
    public function getCanShipAttribute(): bool { return in_array($this->status, ['APPROVED', 'RELEASED', 'PARTIALLY_SHIPPED']); }
    public function getCanInvoiceAttribute(): bool { return in_array($this->status, ['SHIPPED', 'PARTIALLY_INVOICED']); }

    public function getTotalQuantityAttribute(): float { return $this->lines->sum('quantity'); }
    public function getTotalQuantityShippedAttribute(): float { return $this->lines->sum('quantity_shipped'); }
    public function getTotalQuantityToShipAttribute(): float { return $this->lines->sum('quantity_to_ship'); }
