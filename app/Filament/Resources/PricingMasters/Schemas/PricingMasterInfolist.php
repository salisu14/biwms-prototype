            ...
            ->components([
                Section::make('Scope & Applicability')
                    ...
                    ->schema([
                        ...
                        TextEntry::make('price_list_type')->badge()->color('info')->formatStateUsing(fn ($state) => str_replace('_', ' ', $state)),
                        TextEntry::make('customer.name')->label('Customer')->placeholder('All Customers'),
                        TextEntry::make('pricingGroup.name')->label('Pricing Group')->placeholder('-'),
                ...
                Section::make('Pricing Details')
                    ...
                    ->schema([
                        TextEntry::make('price_type')->badge()->color('warning')->formatStateUsing(fn ($state) => str_replace('_', ' ', $state)),
                        TextEntry::make('unit_price')->money('NGN')->visible(fn ($record) => $record->price_type === 'UNIT_PRICE'),
                        TextEntry::make('discount_percent')->suffix('%')->visible(fn ($record) => $record->price_type === 'PERCENT_DISCOUNT'),
                        TextEntry::make('discount_amount')->money('NGN')->visible(fn ($record) => $record->price_type === 'AMOUNT_DISCOUNT'),
                        ...
                        TextEntry::make('minimum_order_amount')->money('NGN')->placeholder('-'),
                ...
                Section::make('Status & Audit')
                    ...
                    ->schema([
                        TextEntry::make('status')->badge()->color(fn ($state) => match($state) { 'ACTIVE' => 'success', 'DRAFT' => 'gray', 'EXPIRED' => 'danger', default => 'warning' }),
