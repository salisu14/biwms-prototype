...
namespace App\Filament\Resources\SalesCreditMemos\Concerns;
...
trait InteractsWithSalesCreditMemoApplications
{
    protected function applyCreditMemoAction(): Action
    {
        return Action::make('applyCreditMemo')
            ...
            ->schema([
                TextInput::make('customer_display')
                    ->label('Customer')
                    ->default(fn (): string => $this->applicationPostedSalesCreditMemo()?->customer_name ?: ($this->applicationPostedSalesCreditMemo()?->customer?->name ?? 'Unknown Customer'))
                    ->disabled()
                    ->dehydrated(false),
                TextInput::make('credit_memo_display')
                    ->label('Credit Memo')
                    ->default(fn (): string => $this->applicationPostedSalesCreditMemo()?->document_number ?? 'Posted Sales Credit Memo')
                    ->disabled()
                    ->dehydrated(false),
                TextInput::make('available_credit_display')
                    ->label('Available Credit')
                    ->default(fn (): string => $this->formatCreditMemoMoney((float) ($this->applicationPostedSalesCreditMemo()?->remaining_amount ?? 0)))
                    ->disabled()
                    ->dehydrated(false),
                Select::make('document_type')
                    ...
                    ->disabled()
                    ->dehydrated(false),
                Placeholder::make('no_eligible_invoice_message')
                    ->label('No eligible invoices')
                    ->content('No eligible open invoices are available for this customer.')
                    ->visible(fn (): bool => $this->eligiblePostedInvoices()->isEmpty()),
        ...
        return $this->eligiblePostedInvoices()
            ->mapWithKeys(fn (PostedSalesInvoice $invoice): array => [
                $invoice->id => trim(implode(' • ', array_filter([
                    ...
                    'Remaining '.$this->formatCreditMemoMoney((float) $invoice->remaining_amount),
                ]))),
            ])
            ->all();
    }
    ...
    {
        $postedCreditMemo = $this->applicationPostedSalesCreditMemo();
        $user = auth()->user();

        if (! $postedCreditMemo || ! $user?->can('sales.credit_memo.apply')) {
            return false;
        }

        if ($postedCreditMemo->corrected || $postedCreditMemo->refunded || $postedCreditMemo->fully_applied) {
            return false;
        }

        if ((float) $postedCreditMemo->remaining_amount <= 0.0001) {
            return false;
        }
    ...
    public function selectedInvoiceSnapshot(mixed $invoiceId): array
    {
        ...
        return [
            ...
            'original_amount' => $this->formatCreditMemoMoney((float) $invoice->grand_total),
            'remaining_amount' => $this->formatCreditMemoMoney((float) $invoice->remaining_amount),
        ];
    }
    ...
    public function applicationLimitForInvoice(mixed $invoiceId): ?float
    {
        ...
        if (! $postedCreditMemo || ! is_numeric($invoiceId)) {
            return null;
        }
        ...
        return round(min((float) $postedCreditMemo->remaining_amount, (float) $invoice->remaining_amount), 4);
    }
    ...
    public function amountHelperText(mixed $invoiceId): string
    {
        ...
        return 'Maximum applicable amount: '.$this->formatCreditMemoMoney($limit).'.';
    }

    /**
     * @return Collection<int, PostedSalesInvoice>
     */
    public function eligiblePostedInvoices(): Collection
    {
        $postedCreditMemo = $this->applicationPostedSalesCreditMemo();
        ...
        $creditMemoBusinessId = $this->businessIdForDocument($postedCreditMemo);
        $creditMemoCurrencyCode = $postedCreditMemo->currency_code ?: null;
        $creditMemoCurrencyId = $postedCreditMemo->currency_id ?? null;

        return PostedSalesInvoice::query()
            ->where('customer_id', $postedCreditMemo->customer_id)
    ...
    protected function sourceCustomerLedgerEntry(): ?CustomerLedgerEntry
    {
        $postedCreditMemo = $this->applicationPostedSalesCreditMemo();
        ...
        return CustomerLedgerEntry::query()
            ...
            ->where('source_id', $postedCreditMemo->id)
            ->first();
    }

    protected function invoiceCustomerLedgerEntry(PostedSalesInvoice $invoice): ?CustomerLedgerEntry
    {
        return CustomerLedgerEntry::query()
    ...
    protected function businessIdForDocument(PostedSalesCreditMemo|PostedSalesInvoice $document): ?int
    {
     ...
     */
    protected function applyCreditMemoToInvoice(array $data): void
    {
        $postedCreditMemo = $this->applicationPostedSalesCreditMemo();

        if (! $postedCreditMemo || ! $this->canApplyCreditMemo()) {
            throw new AuthorizationException('You are not allowed to apply this sales credit memo.');
        }

        try {
            $postedCreditMemo->applyToInvoices([
                [
            ...
            $this->refreshAfterCreditMemoApplication($postedCreditMemo->fresh());
            ...
            Log::error('Posted sales credit memo application failed.', [
                'posted_sales_credit_memo_id' => $postedCreditMemo->id,
    ...
    protected function formatCreditMemoMoney(float $amount): string
    {
        $postedCreditMemo = $this->applicationPostedSalesCreditMemo();

        return Number::currency($amount, $postedCreditMemo?->currency_code ?: config('app.default_currency', 'NGN'));
    }

    abstract protected function applicationPostedSalesCreditMemo(): ?PostedSalesCreditMemo;

    abstract protected function refreshAfterCreditMemoApplication(?PostedSalesCreditMemo $postedCreditMemo): void;
}
